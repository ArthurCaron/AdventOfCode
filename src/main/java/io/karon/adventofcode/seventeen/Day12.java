...
class Day12 {
	...
	static class Star1 {
		...
		private static int getFormattedInput() {
			...
			return exploreLinks(firstNode, links, nodes) + 1;
		}
	...
	private static int exploreLinks(Node node, Map<String, List<String>> links, Map<String, Node> nodes) {
		int counter = 0;
		List<String> currentNodeLinks = links.get(node.id);
		for (String currentNodeLink : currentNodeLinks) {
			Node currentNode = nodes.get(currentNodeLink);
			if (!currentNode.visited) {
				currentNode.visited = true;
				counter++;
				counter += exploreLinks(currentNode, links, nodes);
			}
		}
		return counter;
	}
