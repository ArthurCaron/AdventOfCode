...
class Day7 {
	...
	static class Star1 {
		...
		static String getResult() {
			Node anyNode = getInputAsTree();

			Node root = null;
			while (root == null) {
				Node parent = anyNode.parent;
				if (parent == null) {
					root = anyNode;
				} else {
					anyNode = parent;
	...
	static class Star2 {
		...
		static int getResult() {
			...
			root.calculateRealWeight();

			return root.getExpectedWeightForEquilibrium();
		}
	...
	static class Node {
		...
		Node(String[] splittedLine) {
			this.name = splittedLine[0];
			this.weight = Integer.parseInt(splittedLine[1]);
		...
		void calculateRealWeight() {
			for (Node child : children.values()) {
				child.calculateRealWeight();
				weightOfChildren += child.realWeight;
			}
			realWeight = weight + weightOfChildren;
		}

		int getExpectedWeightForEquilibrium() {
			Map<Integer, List<Node>> weightOccurrence = new HashMap<>();

			for (Node child : children.values()) {
				int diffForEquilibrium = child.getExpectedWeightForEquilibrium();
				if (diffForEquilibrium != 0) {
					return diffForEquilibrium;
				}
			}

			for (Node child : children.values()) {
				List<Node> tmp = weightOccurrence.getOrDefault(child.realWeight, new ArrayList<>());
				tmp.add(child);
				weightOccurrence.put(child.realWeight, tmp);
			}

			int expectedWeight = 0;
			Node wrongNode = null;
			boolean foundWrongWeight = false;
			for (Map.Entry<Integer, List<Node>> entry : weightOccurrence.entrySet()) {
				if (entry.getValue().size() > 1) {
					expectedWeight = entry.getValue().get(0).realWeight;
				} else {
					foundWrongWeight = true;
					wrongNode = entry.getValue().get(0);
				}
			}

			if (foundWrongWeight) {
				return expectedWeight - wrongNode.weightOfChildren;
			} else {
				return 0;
			}
		}
	...
	private static Node getInputAsTree() {
		Map<String, Node> leafNodes = new HashMap<>();
		Map<String, Node> parentNodes = new HashMap<>();

		for (String line : getFormattedInput()) {
			String[] splittedLine = line.split("\\W+");
			Node newNode = new Node(splittedLine);

			if (splittedLine.length == 2) {
				leafNodes.put(newNode.name, newNode);
			} else {
				parentNodes.put(newNode.name, newNode);
			}
		}

		return buildTree(leafNodes, parentNodes);
	}

	private static Node buildTree(Map<String, Node> leafNodes, Map<String, Node> parentNodes) {
		Node anyNode = null;

		for (Node parentNode : parentNodes.values()) {
			anyNode = anyNode == null ? parentNode : anyNode;
			for (Map.Entry<String, Node> entry : parentNode.children.entrySet()) {
				Node child = entry.getValue();
				String childName = entry.getKey();

				if (child == null) {
					Node leaf = leafNodes.get(childName);
					if (leaf != null) {
						parentNode.children.put(childName, leaf);
						leaf.parent = parentNode;
						leafNodes.remove(childName);
					} else {
						Node parent = parentNodes.get(childName);
						if (parent != null) {
							parentNode.children.put(childName, parent);
