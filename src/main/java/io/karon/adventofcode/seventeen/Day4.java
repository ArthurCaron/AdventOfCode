...
class Day4 {
	...
	static class Star1 {
		...
		static int getResult() {
			...
			for (String passphrase : getFormattedInput()) {
		...
		private static boolean goodPassphrase(String passphrase) {
			String[] words = passphrase.split(" ");

			Map<String, Integer> wordComparator = new HashMap<>();
			for (String word : words) {
				if (wordComparator.containsKey(word)) {
					return false;
				} else {
					wordComparator.put(word, 0);
				}
			}
			return true;
		}
	...
	static class Star2 {
		...
		private static boolean goodPassphrase(String passphrase) {
			...
			for (String word : words) {
				...
				if (wordComparator.containsKey(sortedWord)) {
					return false;
				} else {
					wordComparator.put(sortedWord, 0);
				}
			}
			return true;
		}
