...
class Day20 {
	...
	static class Star1 {
		...
		static int getResult() {
			List<Particle> particles = getFormattedInput();
			...
			for (int i = 1; i < particles.size(); i++) {
				...
				if (accelerationManhattanOfSlowest > accelerationManhattan) {
					...
					accelerationManhattanOfSlowest = accelerationManhattan;
				}
			}

			return slowestParticleIndex;
		}
	...
	static class Star2 {
		...
		static int getResult() {
			...
			boolean allGoOppositeToCenter = false;
			while (!allGoOppositeToCenter) {
				updateAll(particles);
				particles = getNonCollidingParticles(particles);

				allGoOppositeToCenter = true;
				for (Particle particle : particles) {
					allGoOppositeToCenter &= particle.goesOppositeToCenter();
				}
			}
		...
		private static List<Particle> getNonCollidingParticles(List<Particle> particles) {
			...
			for (int i = 0; i < particles.size(); i++) {
				...
				if (!collides) {
					outputParticles.add(particles.get(i));
				}
			}
	...
	private static class Particle {
		...
		Particle(int index, String data) {
			this.index = index;
			String[] particleData = data.replaceAll("[pva <=>]", "").split(",");
			position = new Vector(Integer.parseInt(particleData[0]), Integer.parseInt(particleData[1]), Integer.parseInt(particleData[2]));
			velocity = new Vector(Integer.parseInt(particleData[3]), Integer.parseInt(particleData[4]), Integer.parseInt(particleData[5]));
			acceleration = new Vector(Integer.parseInt(particleData[6]), Integer.parseInt(particleData[7]), Integer.parseInt(particleData[8]));
		}
		...
		long accelerationManhattan() {
			return Math.abs(acceleration.x) + Math.abs(acceleration.y) + Math.abs(acceleration.z);
		}

		boolean goesOppositeToCenter() {
			return sameSignX() && sameSignY() && sameSignZ();
		}

		private boolean sameSignX() {
			return (acceleration.x == 0) || (acceleration.x < 0 && velocity.x < 0 && position.x < 0) || (acceleration.x > 0 && velocity.x > 0 && position.x > 0);
		}

		private boolean sameSignY() {
			return (acceleration.y == 0) || (acceleration.y < 0 && velocity.y < 0 && position.y < 0) || (acceleration.y > 0 && velocity.y > 0 && position.y > 0);
		}

		private boolean sameSignZ() {
			return (acceleration.z == 0) || (acceleration.z < 0 && velocity.z < 0 && position.z < 0) || (acceleration.z > 0 && velocity.z > 0 && position.z > 0);
		}
	...
	private static List<Particle> getFormattedInput() {
	...
	private static String getInput() {
		return "p=<-3770,-455,1749>, v=<-4,-77,53>, a=<11,7,-9>\n"
