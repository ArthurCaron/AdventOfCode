...
class Day8 {
	...
	private static final int CONDITION_REGISTER_INDEX = 4;
	private static final int CONDITION_COMPARATOR_INDEX = 5;
	private static final int CONDITION_VALUE_INDEX = 6;

	static class Star1 {

		static int getResult() {
			List<Instruction> instructions = getFormattedInput();
			Map<String, Integer> registers = new HashMap<>();

			for (Instruction instruction : instructions) {
				if (conditionIsTrue(registers, instruction.condition)) {
					resolveInstruction(registers, instruction);
				}
			}

			return getBiggestRegisterValue(registers);
		}
	...
	static class Star2 {
		...
		static int getResult() {
			...
			int biggestRegisterValue = Integer.MIN_VALUE;
			for (Instruction instruction : instructions) {
				if (conditionIsTrue(registers, instruction.condition)) {
					resolveInstruction(registers, instruction);
				}
				biggestRegisterValue = Math.max(biggestRegisterValue, getBiggestRegisterValue(registers));
			}
	...
	private static boolean conditionIsTrue(Map<String, Integer> registers, Instruction.Condition condition) {
		int registerValue = registers.getOrDefault(condition.register, 0);
	...
	private static void resolveInstruction(Map<String, Integer> registers, Instruction instruction) {
		int registerValue = registers.getOrDefault(instruction.register, 0);

		if ("inc".equals(instruction.operation)) {
			registers.put(instruction.register, registerValue + instruction.value);
		} else if ("dec".equals(instruction.operation)) {
			registers.put(instruction.register, registerValue - instruction.value);
		}
	}

	private static int getBiggestRegisterValue(Map<String, Integer> registers) {
	...
	private static class Instruction {
		...
		Instruction(String register, String operation, int value, Condition condition) {
		...
		private static class Condition {
			...
			Condition(String register, String comparator, int value) {
				this.register = register;
	...
	private static List<Instruction> getFormattedInput() {
		...
		for (String line : lines) {
			...
			if (splittedLine.length < 7) {
				throw new IllegalArgumentException("line doesn't contain all necessary elements");
			}

			instructions.add(new Instruction(
					splittedLine[REGISTER_INDEX],
					splittedLine[OPERATION_INDEX],
					Integer.parseInt(splittedLine[VALUE_INDEX]),
					new Instruction.Condition(
						splittedLine[CONDITION_REGISTER_INDEX],
						splittedLine[CONDITION_COMPARATOR_INDEX],
						Integer.parseInt(splittedLine[CONDITION_VALUE_INDEX]))));
		}
